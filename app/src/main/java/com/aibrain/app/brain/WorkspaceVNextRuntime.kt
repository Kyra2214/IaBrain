...
class WorkspaceVNextRuntime(context: Context) {
    ...
    fun recordPromptAction(prompt: String, aiId: String?, aiName: String?, action: PromptActionType): PromptActionRecord {
        ...
        appendJson(KEY_PROMPT_ACTIONS, record.toJson()); return record
    }
    ...
    fun runSkill(projectId: String, skill: SkillDefinition, context: String): SkillRun { val run = IaBrainWorkspaceOrchestrator.runSkill(skill, context); appendJson(KEY_SKILL_RUNS, JSONObject().apply { put("projectId", projectId); put("skillId", run.skillId); put("context", run.context); put("steps", JSONArray(run.steps)); put("automaticSend", run.automaticSend); put("at", System.currentTimeMillis()) }); return run }
    ...
    fun createTask(title: String, projectId: String?, priority: TaskPriority = TaskPriority.NORMAL): ProjectTask { val task = IaBrainWorkspaceOrchestrator.createTask(title, projectId, priority); appendJson(KEY_TASKS, task.toJson()); return task }
    ...
    fun updateTaskStatus(id: String, status: TaskStatus): Boolean { val updated = tasks().map { if (it.id == id) it.copy(status = status) else it }; if (updated.none { it.id == id }) return false; prefs.edit().putString(KEY_TASKS, JSONArray(updated.map { it.toJson().toString() }).toString()).apply(); return true }
    ...
    private fun readJson(key: String): List<JSONObject> { val array = runCatching { JSONArray(prefs.getString(key, "[]")) }.getOrDefault(JSONArray()); return (0 until array.length()).mapNotNull { runCatching { JSONObject(array.getString(it)) }.getOrNull() } }
...
data class PromptActionRecord(val id: String, val prompt: String, val aiId: String?, val aiName: String?, val action: PromptActionType, val createdAt: Long) {
    fun toJson() = JSONObject().apply { put("id", id); put("prompt", prompt); put("aiId", aiId); put("aiName", aiName); put("action", action.name); put("createdAt", createdAt) }
...
private fun parseWorkspaceContext(o: JSONObject) { val a=o.optJSONArray("resources") ?: JSONArray(); return ProjectWorkspaceContext(o.optString("projectId"), o.optString("projectName"), o.optString("stack"), o.optString("objective"), o.optString("currentState"), (0 until a.length()).map { a.optString(it) }) }
private fun parseChatContext(o: JSONObject): ProjectChatContext { val files=o.optJSONArray("selectedFiles") ?: JSONArray(); val contributions=o.optJSONArray("selectedContributions") ?: JSONArray(); val memories=o.optJSONArray("recentMemory") ?: JSONArray(); return ProjectChatContext(parseWorkspaceContext(o.optJSONObject("workspace")), (0 until files.length()).map { files.optString(it) }, (0 until contributions.length()).map { contributions.optString(it) }, (0 until memories.length()).map { parseMemory(JSONObject(memories.optString(it))) }) }

...
class IAUrlResolver(context: Context) {
    ...
    suspend fun resolve(contract: IAOpenContract): IAOpenContract {
        ...
        return contract.copy(officialResolvedUrl = if (valid) url else null, urlStatus = if (valid) UrlResolutionStatus.RESOLVED else if (url == null) UrlResolutionStatus.NOT_FOUND else UrlResolutionStatus.INVALID)
    }
}

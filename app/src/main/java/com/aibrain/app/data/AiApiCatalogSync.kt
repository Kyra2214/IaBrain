...
/** Synchronizes the API catalog at most once per 24h. Network data is never required for app startup. */
class AiApiCatalogSync(context: Context) {
    private val appContext = context.applicationContext
    private val db = AppDatabase.getInstance(appContext)
    private val scope = CoroutineScope(SupervisorJob() + Dispatchers.IO)

    fun syncIfStale(nowEpochMs: Long = System.currentTimeMillis()) {
        val state = runCatching { db.aiApiSyncStateDao().get() }.getOrNull()
        if (!com.aibrain.app.brain.AiApiSyncPolicy.isStale(state?.lastSyncAt ?: 0L, nowEpochMs)) return
        scope.launch { sync(nowEpochMs) }
    }

    fun syncNow(nowEpochMs: Long = System.currentTimeMillis()): Result<Int> = runCatching {
        syncBlocking(nowEpochMs)
    }

    private fun sync(nowEpochMs: Long) {
        runCatching { syncBlocking(nowEpochMs) }
    }

    private fun syncBlocking(nowEpochMs: Long): Int {
        val raw = fetchRemoteCatalog() ?: appContext.assets.open(LOCAL_ASSET).bufferedReader().use { it.readText() }
        val hash = sha256(raw)
        val root = JSONObject(raw)
        val version = root.optString("version", "unknown")
        val providers = root.optJSONArray("providers") ?: return 0
        val entities = buildList {
            for (i in 0 until providers.length()) {
                ...
                val region = provider.optString("region", "GLOBAL")
                val documentationUrl = provider.optString("documentationUrl", provider.optString("officialUrl"))
                ...
                for (j in 0 until models.length()) {
                    ...
                    val endpoint = model.optString("endpoint")
                    val capabilities = model.optJSONArray("capabilities") ?: continue
        ...
        if (entities.isNotEmpty()) db.aiApiCatalogDao().upsertAll(entities)
        db.aiApiSyncStateDao().save(AiApiSyncStateEntity(1, nowEpochMs, version, hash))
        return entities.size
    }

    private fun fetchRemoteCatalog(): String? = runCatching {
    ...
    companion object {
        private const val LOCAL_ASSET = "ai_api_catalog.json"
        private const val REMOTE_URL = "https://raw.githubusercontent.com/Kyra2214/IaBrain/main/app/src/main/assets/ai_api_catalog.json"
    }
}

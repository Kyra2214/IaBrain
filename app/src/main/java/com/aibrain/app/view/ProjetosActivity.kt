...
class ProjetosActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        cabecalho.addView(TextView(this).apply {
            text = getString(R.string.projetos_titulo)
            textSize = 26f
            setTextColor(getColor(R.color.on_background))
            setTypeface(null, android.graphics.Typeface.BOLD)
        }, LinearLayout.LayoutParams(0, -2, 1f))
        cabecalho.addView(Button(this).apply {
            text = getString(R.string.projetos_novo)
            setOnClickListener { startActivity(Intent(this@ProjetosActivity, CriarComIAActivity::class.java)) }
        })
        raiz.addView(cabecalho)
        raiz.addView(TextView(this).apply {
            text = getString(R.string.projetos_descricao)
            setTextColor(getColor(R.color.on_background_muted))
            setPadding(0, 4, 0, 10)
        })
        raiz.addView(Button(this).apply {
            text = "🧠 Workspace 2.0 · 12 blocos"
            setOnClickListener { startActivity(WorkspaceVNextActivity.criarIntent(this@ProjetosActivity)) }
        })
        raiz.addView(Button(this).apply {
            text = "📋 Task Center"
            ...
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                projetoRepository.observarTodos().collect { projetos ->
                    ...
                    projetos.forEach { projeto ->
                        val card = LinearLayout(this@ProjetosActivity).apply {
                            orientation = LinearLayout.VERTICAL
                            setPadding(16, 16, 16, 16)
                            setBackgroundResource(R.drawable.bg_card)
                        }
                        card.addView(TextView(this@ProjetosActivity).apply {
                            text = projeto.nome
                            textSize = 19f
                            setTextColor(getColor(R.color.on_background))
                            setTypeface(null, android.graphics.Typeface.BOLD)
                        ...
                        card.addView(TextView(this@ProjetosActivity).apply {
                            text = "${projeto.plataforma ?: "Stack não definida"} · ${projeto.complexidade}\n${projeto.status}"
                            setTextColor(getColor(R.color.on_background_muted))
                            setPadding(0, 6, 0, 8)
                        })
                        card.addView(Button(this@ProjetosActivity).apply {
                            text = getString(R.string.projetos_abrir)
                            setOnClickListener { startActivity(ProjetoDetalheActivity.criarIntent(this@ProjetosActivity, projeto.id)) }
                        })

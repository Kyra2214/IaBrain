 ...
 * A memória nunca é enviada para uma IA por esta tela.
 */
class ProjetoMemoriaActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        root.addView(TextView(this).apply {
            text = "🧠 Memória do projeto"
            textSize = 24f
            setTypeface(null, android.graphics.Typeface.BOLD)
            setTextColor(getColor(R.color.on_background))
        })
        root.addView(TextView(this).apply {
            text = "Registre decisões, arquitetura, problemas e soluções para reutilização no contexto do Chat."
            textSize = 15f
            setTextColor(getColor(R.color.on_background))
            setPadding(0, 8, 0, 12)
        })
        root.addView(Button(this).apply {
            text = "➕ Nova memória"
            setOnClickListener { criarMemoria() }
        })
    ...
    private fun carregar() {
        lifecycleScope.launch {
            ...
            val pid = projetoId
            if (pid == null) {
                lista.addView(TextView(this@ProjetoMemoriaActivity).apply {
            ...
            memorias.forEach { memoria -> adicionarCard(memoria) }
        }
    }
    ...
    private fun adicionarCard(memoria: ProjetoMemoriaEntity) {
        ...
        card.addView(TextView(this).apply {
            text = "${memoria.tipo} · ${memoria.titulo}"
            textSize = 17f
            setTypeface(null, android.graphics.Typeface.BOLD)
        })
        card.addView(TextView(this).apply {
            text = memoria.conteudo
            textSize = 15f
            setPadding(0, 8, 0, 8)
        })
        card.addView(TextView(this).apply {
            text = "Atualizada: ${java.text.DateFormat.getDateTimeInstance().format(java.util.Date(memoria.atualizadoEm))}"
            textSize = 12f
        })
        card.addView(Button(this).apply {
            text = "🗑 Remover"
            setOnClickListener {
                AlertDialog.Builder(this@ProjetoMemoriaActivity)
                    .setTitle("Remover memória?")
                    .setMessage(memoria.titulo)
                    .setNegativeButton("Cancelar", null)
                    .setPositiveButton("Remover") { _, _ ->
                        lifecycleScope.launch { repository.removerMemoria(memoria); carregar() }
                    }.show()
            }
        })
    ...
    private fun criarMemoria() {
        ...
        AlertDialog.Builder(this)
            ...
            .setSingleChoiceItems(tipos, 0) { _, which -> tipo = tipos[which] }
            .setView(box)
            .setNegativeButton("Cancelar", null)
            .setPositiveButton("Salvar") { _, _ ->
                val t = titulo.text.toString().trim()
                val c = conteudo.text.toString().trim()
                if (t.isBlank() || c.isBlank()) { toast("Título e conteúdo são obrigatórios"); return@setPositiveButton }
                lifecycleScope.launch {
                    val now = System.currentTimeMillis()
                    repository.salvarMemoria(ProjetoMemoriaEntity(UUID.randomUUID().toString(), pid, tipo, t, c, now, now))
                    carregar()
                }
            }
            .show()
    ...
    companion object {
        ...
        fun criarIntent(context: android.content.Context, projectId: String) =
            android.content.Intent(context, ProjetoMemoriaActivity::class.java).putExtra(EXTRA_PROJECT_ID, projectId)
    }
}

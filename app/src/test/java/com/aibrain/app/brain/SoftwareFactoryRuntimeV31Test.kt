...
class SoftwareFactoryRuntimeV31Test {
    ...
    fun baseModifiedPolicyBlocksByDefaultAndAllowsOnlyExplicitOverride() {
        ...
        val runtime = SoftwareFactoryRuntimeV31()
        val blocked = runtime.merge(root, analysis, listOf(artifact), mapOf("implementation" to setOf("A.kt")))
        assertTrue(blocked.blocked)
        assertTrue(!blocked.merged)
        assertTrue(blocked.reason!!.contains("BASE_MODIFIED"))
    ...
    fun persistentStoreSurvivesNewInstanceAndPipelineEmitsProjectId() {
        ...
        pipeline.receiveZip(plan, SoftwareFactoryPipeline.Submission(item.taskId, item.functionId, "ia", zip), declaredFilesByFunction = mapOf(item.functionId to setOf("analysis.txt")), allowAutoMerge = true)
        assertTrue(File(workspace, "analysis.txt").isFile)
        assertEquals(SoftwareFactoryIntelligenceV31.MergeDecision.AUTO_MERGE, pipeline.lastMergeResult()!!.assessment.decision)
